    ...
    try:
        ...
        if response.status_code == requests.codes.ok:
            ...
            bg_colors = {
                ...
                "蓝色": (0, 0, 255),
                "白色": (255, 255, 255),
                "蓝色渐变": add_gradient_background,
            }
            if bg_color in bg_colors:
                if callable(bg_colors[bg_color]):
                    result_image = bg_colors[bg_color](result_image)
                else:
                    result_image = add_background_color(result_image, bg_colors[bg_color])
